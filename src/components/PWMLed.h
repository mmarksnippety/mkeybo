...
public:
    explicit PWMLed(uint8_t pwm_pin, uint8_t pwm_duty, uint16_t pwm_frequency = 1000)
    ...
    void pwm_setup_freq() {
        uint32_t clock = 125000000;// TODO: get automatically
        pwm_divider16_ = clock / pwm_frequency_ / 4096 + (clock % (pwm_frequency_ * 4096) != 0);
    ...
    void blink(uint32_t interval_ms, uint8_t duty = 100, uint8_t blink_max = 1) {
