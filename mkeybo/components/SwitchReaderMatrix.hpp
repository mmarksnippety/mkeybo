...
namespace mkeybo {
    ...
    public:
        explicit SwitchReaderMatrix(
            ...
            rows_bit_mask_ = ((1 << row_count) - 1)
                << row_start_pin; // Row mask is unused, allways read all gpio
        ...
        void read_state(std::bitset<switches_count>& switch_states) override {
            ...
            for (uint8_t current_column = 0; current_column < column_count_; current_column++) {
                ...
                auto rowInput = gpio_get_all();
                rowInput &= rows_bit_mask_;
                rowInput >>= row_start_pin_;
                for (uint8_t current_row = 0; current_row < row_count_; current_row++) {
                    ...
                    switch_states[switch_index] = rowInput & 1;
                    rowInput >>= 1;
                    raw_buffer_mask <<= 1;
