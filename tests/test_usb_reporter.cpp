...
#include "mkeybo/components/usb_reports.hpp"
...
void test_hid_reporter_get_regular_keycodes()
{
    ...
    const auto hid_reporter = new mkeybo::UsbKeyboardReport<switches_count>();
    std::cout << __func__ << "...";
...
void test_hid_reporter_generate_report_modifiers()
{
    ...
    k_state->push_keycode_event(H_K(HID_KEY_SHIFT_LEFT), 0, mkeybo::KeycodeEventType::finalized);
    k_state->push_keycode_event(H_K(HID_KEY_Q), 0, mkeybo::KeycodeEventType::finalized);
    const auto report = new mkeybo::UsbKeyboardReport();
...
void test_reporter_manager_generate_reports()
{
    ...
    const auto usb_hid_report = reinterpret_cast<mkeybo::UsbKeyboardReport*>(k_state->get_usb_reports().at(mkeybo::KeycodeType::hid));
    ...
    const auto usb_cc_report = reinterpret_cast<mkeybo::UsbCcReport*>(k_state->get_usb_reports().at(mkeybo::KeycodeType::cc));
